...
class Planta(models.Model):
    ...
    tiempo_produccion = models.IntegerField(null=True, verbose_name='Tiempo de producción (Semanas)')
...
class Siembra(models.Model):
    ...
    def toJSON(self):
        item = model_to_dict(self)
        item['fecha_siembra'] = self.fecha_siembra.strftime('%d-%m-%Y %H:%M')
...
class HistorialRiego(models.Model):
    ...
    siembra = models.ForeignKey(Siembra, on_delete=models.PROTECT)
    tipo_rol = models.ForeignKey(TipoRol, on_delete=models.PROTECT)
    tipo_logica_difusa = models.ForeignKey(TipoLogicaDifusa, on_delete=models.PROTECT)
